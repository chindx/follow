...
window.onload = function () {	
	bind("regName", "blur", function () {
		validateName();
	});
	bind("name", "blur", function () {
		validateName();
	});

	bind("password", "blur", function() {
		validatePass();
	});
	
	bind("repass", "blur", function() {
		validateRepass();
	});	
	
	bind("user_reg", "submit", function(e) {
		if(!validate()) {  // 验证没有通过
...
function validateRegName() {  
    return validateRegex("regName", /^\w{11}$/ | /[\\w,-]+(\\.([\\w,-])+)*@[\\w,-]+(\\.[\\w,-]+)+)/)
}
/** 验证是否为空 */
...
function validatePass() {
	return validateRegex("password", /^\w{8,20}$/) ;
}

/** 确认密码的验证 */
function validateRepass() {
	return validateRepeat("repass", "password");
} 

function validate() {
	return validateRegName() & validateName() & validatePass() & validateRepass();
}